...
using testeUC;
using texteRecette;
using TexteRecetteEtape;

namespace projet_SEILER_TRAN_VOLTZ
{
    public partial class Etape : Form
    {
        public Etape()
        {
            InitializeComponent();
        ...
        //*********************************************
        //generation de la page de la recette par Etape
        //*********************************************
        private void Etape_Load(object sender, EventArgs e)
        {
            ...
            //on appele la méthode pour nous remplir le bindingSource
            generationBidingSource();
            bindingSource.MoveFirst();
            affichageEtape();
        }

        private void etapeSuivant(object sender, EventArgs e)
        {            
        ...
        private void etapePrecedant(object sender, EventArgs e)
        {
        ...
        private void affichageEtape()
        {
            DataRowView ligne = (DataRowView)bindingSource.Current;
            txtRecetteEtape[] bt = this.Controls.OfType<txtRecetteEtape>().ToArray();

            foreach (txtRecetteEtape b in bt)
            {
            ...
            if (ligne != null )
            {
                string image = "Image/" + ligne["imageEtape"].ToString();
                string texte = ligne["texteEtape"].ToString();
                string etape = ligne["numEtape"].ToString();

                txtRecetteEtape afficheEtape = new txtRecetteEtape(image, etape,totEtape,texte, 850,350);
                afficheEtape.Parent = this;
                afficheEtape.Location = new Point(60,270);
            }
        }

        private void generationBidingSource()
        {
            ...
            for (int i = 1; i < (int)dtEtape.Rows[0][0] + 1; i++)
            {
                ...
                string img = @"SELECT imageEtape,texteEtape ,numEtape  FROM EtapesRecette
                           WHERE codeRecette = " + numRecette + " AND numEtape = " + i;
                
                OleDbDataAdapter dataB = new OleDbDataAdapter(img, connec);
                dataB.Fill(dtBinding);
            }
            bindingSource.DataSource = dtBinding;
            //dataGridView1.DataSource = bindingSource;
            connec.Close();
        }

        private void afficherRecap(int type)
        {
            recapRecette[] bt = this.Controls.OfType<recapRecette>().ToArray();

            foreach (recapRecette b in bt)
            {
                this.Controls.Remove(b);
            }
            ...
            data.Fill(dt);
            recapRecette recap = new recapRecette(dt.Rows[0][0].ToString(), dt.Rows[0][2].ToString() + " minutes", dt.Rows[0][1].ToString() + " personnes", (int)dt.Rows[0][3], 550, 190, type);
